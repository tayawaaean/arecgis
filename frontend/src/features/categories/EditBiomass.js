...
import { Status, rawBioUsage, rawBiomassDigester, rawBiomassLoc, rawBiomassPriUsage, rawBiomassSystemStatus } from '../../config/techAssesment'
import { boxstyle } from '../../config/style'


export const EditBiomass = (props) => {

    const [data, setData] = useState([])
    
    let biomassUse = props?.reItems?.assessment?.biomassPriUsage

    let found = rawBiomassPriUsage.findIndex(item => item.name === biomassUse)

    const [capacity, setCapacity] = useState(props?.reItems?.assessment?.capacity || '')

    const [biomassPriUsage, setBiomassPriUsage] = useState(found === -1 ? { index: '', value: 'Other', otherVal: biomassUse } : { index: '', value: biomassUse, otherVal: '' })

    const [bioUsage, setBioUsage] = useState({ index: '', value: props?.reItems?.assessment?.bioUsage || '', otherVal: '' })
